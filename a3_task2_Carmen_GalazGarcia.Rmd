...
```{r}
# Your code here: 
sw_split_name <- starwars %>%           # coy data
  separate(col=name,                    # separate name columns
           into = c("name_1", "name_2"),# names of new columns
           sep="-")                     # separation at first dash
```
...
```{r}
# Your code here:
sw_skin_sep <- starwars %>%             # copy dataset
  separate(col=skin_color,              # separate skin_color column
           into=c("skin_1", "skin_2"),  # names of new columns
           sep=",",                     # separator is "-" (dash)
           remove = FALSE,              # keep original column
           extra = "merge")             # merge everything following - into the second column
...
```{r}
# Your code here: 
mpg_model_class <- mpg %>%   # copy data
  unite(col= model_class,    # create a column called model_class
        c(model,class),      # that combines the model and class columns
        sep = "-",           # merge columns using "-" between data
...
# Your code here: 
 ...
 # convert my birthday in different formats to Date class:
ymd("1991-05-21") 
...
```{r}
# Your code here: 
some_dates <- c(04011953, 04031952, 03271979)  # dates of loved ones as numbers
some_dates_converted<-mdy(some_dates)          # convert to date format
some_dates_converted                           # show dates in console
class(some_dates_converted)                    # check class is Date
```
...
```{r}
# Your code here: 
gasol_hookshots <- lakers %>%      # copy lakers data
  filter(player == "Pau Gasol" & type=="hook") %>%   # choose Pau Gasol hooks data 
  mutate(new_date = ymd(date)) %>%                   # add column new_date with dates converted to Date class
  mutate(new_month = month(new_date)) %>%            # add column new_month extracting month from new_date
  count(new_month)                # count number of Pau Gasol's hooks per month

gasol_hookshots    # show number of Pau Gasol's hooks per month
...
```{r}
# Your code here: 
storms_danger <- storms %>%           # copy storms dataset
  mutate(danger = case_when(          # add column danger, which contains...
    status == "hurricane"~"high",     # "high" if status equals hurricane
    status != "hurricane"~"moderate"  # "moderate" otherwise
  ))
